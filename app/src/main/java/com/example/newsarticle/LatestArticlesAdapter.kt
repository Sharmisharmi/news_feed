...
class LatestArticlesAdapter (context: Context, data: List<getArticleData>?) : RecyclerView.Adapter<LatestArticlesAdapter.ViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): LatestArticlesAdapter.ViewHolder {
        return ViewHolder(LayoutInflater.from(parent.context).inflate(R.layout.recent_articles_list,parent,false))
    }

    override fun onBindViewHolder(holder: LatestArticlesAdapter.ViewHolder, position: Int) {

        holder.newsTitle.text=data[position].author.toString()
